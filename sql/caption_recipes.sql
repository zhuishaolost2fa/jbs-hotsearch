...
-- ------------------------------------------------------------
-- 注意：prompt 里的换行用**字面换行**写在一个单引号串里。
-- 不要写成 E'...\n' 多行拼接 —— PostgreSQL 不允许 E'' 前缀的字符串做相邻拼接。
...
请写一段「可以直接抄作业」的小红书清单文案，要求：
1. 第一行是标题，点明日期和城市，让人一眼知道这是今天的杭州榜单
2. 正文按名次列 Top3，每本一句话说清「适合谁 / 为什么值得去」，不要只报排名
3. 数据只能用上面给的，禁止编造热度值、名次或剧本名
4. 结尾一句引导收藏（比如「周末要打的先码住」）
5. 末尾 4-5 个话题标签
{parsed_hint}
直接输出文案，不要任何解释或前后缀。',
