...
# 禁则（代码内置与线上配方共用同一段，改一处即可）：
#   1. 诱导互动 ——「扣1」「求赞」「点赞」「收藏」「关注我」「码住」「蹲一个」
#      「评论区告诉我」这类请求读者做动作的表达，小红书判「诱导互动」违规；
#   2. 站外导流 ——「私信」「微信」「加我」「扫码」「公众号」，判「站外引流」违规。
# 两条都限流。文案只能陈述信息，不能请求任何动作。
# （2026-09-22 踩坑：曾把「评论区扣 1」当作导流的安全替代，结果它踩的是诱导互动这条线）
NO_INDUCE_RULE = (
    "注意：全篇不得请求读者做任何动作（禁止「扣1」「求赞」「点赞」「收藏」「关注我」"
    "「码住」「蹲一个」「评论区告诉我」这类表达 —— 小红书判定为诱导互动违规），"
    "也不得出现「私信」「微信」「加我」「扫码」「公众号」等站外导流词；只陈述信息。"
)
...
def build_context(
    ...
    if caption_parsed:
        ...
        parsed_hint = (
            f"4. 文末用一句话陈述这些已解析剧本的状态：{parsed_names}，"
            "说明它们热度够高但攻略已整理好（DM 手册已入库），本期未入榜。"
            "只陈述事实，不要请求读者做任何动作；"
